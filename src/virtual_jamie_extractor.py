...
class VirtualJamieDataExtractor:
    def __init__(self):
        ...
        # Output database to location accessible by the app
        # Allow overriding via PETE_DB_PATH for flexibility
        default_db_path = self._get_default_db_path()
        self.target_db_path = os.getenv("PETE_DB_PATH", default_db_path)
        
    def _get_default_db_path(self):
        """Get the default database path that works locally and on RunPod"""
        possible_paths = [
            # Local development (current directory) - prioritize for local dev
            os.path.join(os.getcwd(), "pete.db"),
            # Relative to project root
            os.path.join(os.path.dirname(__file__), "..", "pete.db"),
            # RunPod workspace (preferred for RunPod)
            "/app/pete.db",
            # Fallback to temp directory
            os.path.join("/tmp", "pete.db")
        ]
        ...
        # Final fallback
        return os.path.join(os.getcwd(), "pete.db")
        
    def load_environment(self):
        """Load environment variables from local .env file"""
        # Try multiple paths to find .env file (works locally and on RunPod)
        possible_env_paths = [
            # Local development (from src/ to root)
            os.path.join(os.path.dirname(__file__), '..', '.env'),
            # RunPod workspace
            os.path.join('/workspace', 'Pete_ollama_agent', '.env'),
            # Current directory
